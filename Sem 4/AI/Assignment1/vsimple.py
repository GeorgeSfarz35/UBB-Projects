    ...
    def moveDSF(self, detectedMap):
        ...
        if not (self.x, self.y) in self.visited:
            ...
            for direction in [UP, DOWN, LEFT, RIGHT]:
                x_path = self.x + v[direction][0]
                y_path = self.y + v[direction][1]
                if (x_path, y_path) not in self.visited:
                    if 0 <= x_path < 20 and 0 <= y_path < 20:
                        if detectedMap.surface[x_path][y_path] == 0:
                            self.path.append((self.x, self.y))
                            self.path.append((x_path, y_path))
        if len(self.path) != 0:
